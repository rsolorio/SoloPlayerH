 ...
 * Fields: id, name, hash, artistSort, artistStylized, favorite, songCount, playCount, songAddDateMax
 */
@ViewEntity({
  name: 'artistView',
  expression: `
    SELECT artist.id, artist.name, artist.hash, artist.artistSort, artist.artistStylized, artist.favorite, COUNT(partyRelation.songId) AS songCount, 0 AS playCount, NULL AS songAddDateMax
    FROM artist
    LEFT JOIN (
